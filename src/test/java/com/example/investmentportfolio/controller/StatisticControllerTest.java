...
class StatisticControllerTest {
    ...
    @Test
    @WithMockUser(roles = {"ADMIN", "USER"})
    void givenAdminRole_whenGetStatisticsByUserId_thenReturnStatistics() throws Exception {
        StatisticDto responseStatisticDto1 = new StatisticDto("bob_da_builderz", "D05", "SGX", "546.88", "103002.59210", "240112.90900", "30000.45010", "70021.22040", "20100.110", "140001.8302");
    ...
    @Test
    void givenUserRoleWithValidUserId_whenGetStatisticsByUserId_thenReturnStatistics() throws Exception {
    ...
    @Test
    void givenUserRoleWithInvalidUserId_whenGetStatisticsByUserId_thenReturnForbidden() throws Exception {
        Authentication authentication = new UsernamePasswordAuthenticationToken(
                2L,
                null,
                AuthorityUtils.createAuthorityList("ROLE_USER")
    ...
    @Test
    void givenNoRole_whenGetStatisticsByUserId_thenReturnUnauthorized() throws Exception {
        mockMvc.perform(get("/statistics/userId/{userId}", 1)
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isUnauthorized());
    }
    ...
    @Test
    @WithMockUser(roles = {"ADMIN", "USER"})
    void givenValidRequestAndAdminRole_whenUpdateStatisticById_thenReturnOk() throws Exception {
        StatisticDto requestStatisticDto = new StatisticDto("bob_da_builderz", "D05", "SGX", "546.88", "103002.59210", "240112.90900", "30000.45010", "70021.22040", "20100.110", "140001.8302");
        StatisticDto responseStatisticDto = new StatisticDto("bob_da_builderz", "D05", "SGX", "446.88", "103002.59210", "240112.90900", "30000.45010", "70021.22040", "20100.110", "140001.8302");
        when(statisticService.updateStatisticById(any(), any())).thenReturn(responseStatisticDto);
        mockMvc.perform(post("/statistics/update/id/{statisticId}", 1)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(requestStatisticDto))
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.stockTicker").value("D05"))
                .andExpect(jsonPath("$.exchange").value("SGX"))
                .andExpect(jsonPath("$.totalUnits").value("446.88"))
                .andExpect(jsonPath("$.totalCost").value("103002.59210"))
                .andExpect(jsonPath("$.totalValue").value("240112.90900"))
                .andExpect(jsonPath("$.realizedProfits").value("30000.45010"))
                .andExpect(jsonPath("$.unrealizedProfits").value("70021.22040"))
                .andExpect(jsonPath("$.dividendsEarned").value("20100.110"))
                .andExpect(jsonPath("$.totalProfits").value("140001.8302"));
        verify(statisticService, times(1)).updateStatisticById(any(), any());
    }

    @Test
    @WithMockUser(roles = {"ADMIN", "USER"})
    void givenInvalidRequestAndAdminRole_whenUpdateStatisticById_thenReturnBadRequest() throws Exception {
        StatisticDto requestStatisticDto = new StatisticDto("bob_da_builderz", "", "SGX", "546.88", "103002.59210", "240112.90900", "30000.45010", "70021.22040", "20100.110", "140001.8302");
        mockMvc.perform(post("/statistics/update/id/{statisticId}", 1)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(requestStatisticDto))
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.errorCode").value("400 Bad Request"))
                .andExpect(jsonPath("$.errorMessages").value("Stock ticker must only contain letters or numbers and be up to 10 characters."));
    }

    @Test
    @WithMockUser(roles = "USER")
    void givenUserRole_whenUpdateStatisticById_thenReturnForbidden() throws Exception {
        StatisticDto requestStatisticDto = new StatisticDto("bob_da_builderz", "D05", "SGX", "546.88", "103002.59210", "240112.90900", "30000.45010", "70021.22040", "20100.110", "140001.8302");
        mockMvc.perform(post("/statistics/update/id/{statisticId}", 1)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(requestStatisticDto))
    ...
    @Test
    void givenNoRole_whenUpdateStatisticById_thenReturnUnauthorized() throws Exception {
        mockMvc.perform(post("/statistics/update/id/{statisticId}", 1)
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isUnauthorized());
    }
    ...
    @Test
    @WithMockUser(roles = {"ADMIN", "USER"})
    void givenAdminRole_whenDeleteAllStatistics_thenDeleteAllStatistics() throws Exception {
        mockMvc.perform(delete("/statistics/delete/all")
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isOk())
                .andExpect(content().string("Successfully deleted all statistics."));
        verify(statisticService, times(1)).deleteAllStatistics();
    }

    @Test
    @WithMockUser(roles = "USER")
    void givenUserRole_whenDeleteAllStatistics_thenReturnForbidden() throws Exception {
        mockMvc.perform(delete("/statistics/delete/all")
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.errorCode").value("403 Forbidden"))
    ...
    @Test
    void givenNoRole_whenDeleteAllStatistics_thenReturnUnauthorized() throws Exception {
        mockMvc.perform(delete("/statistics/delete/all")
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isUnauthorized());
    }
    ...
    @Test
    @WithMockUser(roles = {"ADMIN", "USER"})
    void givenAdminRole_whenDeleteStatisticById_thenDeleteStatistic() throws Exception {
        mockMvc.perform(delete("/statistics/delete/id/{statisticId}", 1)
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isOk())
                .andExpect(content().string("Successfully deleted statistic with id: 1"));
        verify(statisticService, times(1)).deleteStatisticById(any());
    }

    @Test
    @WithMockUser(roles = "USER")
    void givenUserRole_whenDeleteStatisticById_thenReturnForbidden() throws Exception {
        mockMvc.perform(delete("/statistics/delete/id/{statisticId}", 1)
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.errorCode").value("403 Forbidden"))
    ...
    @Test
    void givenNoRole_whenDeleteStatisticById_thenReturnUnauthorized() throws Exception {
        mockMvc.perform(delete("/statistics/delete/id/{statisticId}", 1)
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isUnauthorized());
    }
    ...
    @Test
    @WithMockUser(roles = {"ADMIN", "USER"})
    void givenAdminRole_whenDeleteStatisticByUserId_thenDeleteStatistics() throws Exception {
        mockMvc.perform(delete("/statistics/delete/userId/{userId}", 1)
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isOk())
                .andExpect(content().string("Successfully deleted statistics with user id: 1"));
        verify(statisticService, times(1)).deleteStatisticsByUserId(any());
    }

    @Test
    @WithMockUser(roles = "USER")
    void givenUserRole_whenDeleteStatisticByUserId_thenReturnForbidden() throws Exception {
        mockMvc.perform(delete("/statistics/delete/userId/{userId}", 1)
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.errorCode").value("403 Forbidden"))
    ...
    @Test
    void givenNoRole_whenDeleteStatisticByUserId_thenReturnUnauthorized() throws Exception {
        mockMvc.perform(delete("/statistics/delete/userId/{userId}", 1)
                        .with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isUnauthorized());
    }
